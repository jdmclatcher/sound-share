...
import { ref, startAt, endAt, query, orderByKey, get, push, set, onValue } from "firebase/database";
  ...
  if (accessToken) {
    getCurrentUserProfile(accessToken).then(async (profile) => {
      ...
      try {
        ...
        const currentUserFriendRef = ref(firebase, `users/${username}/friends`);
        ...
        if (!isFriend(userId) && userId != username) {
          ...
          const friendFriendRef = ref(firebase, `users/${userId}/friends/${username}`);
          const currentUserData = {"name": username};
          await set(friendFriendRef, currentUserData);
          console.log("Friend added successfully");
        } else {
          console.log("User is already a friend or trying to add yourself as friend");
        }
      } catch (error) {
